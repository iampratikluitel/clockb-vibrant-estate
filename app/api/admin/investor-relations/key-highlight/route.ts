    ...
    try {
        ...
        if(user) {
            ...
            if (data?._id) {
                ...
                if(existingConfig.card1icon && existingConfig.card1icon !== data.card1icon) {
                    await minioClient.removeObject(BUCKET_NAME, existingConfig.card1icon);
                }

                if(existingConfig.card1icon && existingConfig.card2icon !== data.card2icon) {
                    await minioClient.removeObject(BUCKET_NAME, existingConfig.card2icon);
                }

                if(existingConfig.card1icon && existingConfig.card3icon !== data.card3icon) {
