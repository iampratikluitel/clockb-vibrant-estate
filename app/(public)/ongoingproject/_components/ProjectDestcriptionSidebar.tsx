...
} from "@/store/api/Public/publicProject";

const ProjectDescriptionSidebar = () => {
  const { slug } = useParams();

  const { data: Project } = useGetPublicProjectQuery("");
  const { data: ProjectBySlug } = useGetPublicProjectBySlugQuery(
    slug as string
  );

  const { data: ProjectCategory } = useGetPublicProjectByCategoryQuery("");

  const recentPosts = Project
    ? Project.filter((project) => project._id !== ProjectBySlug?._id).slice(
        0,
        5
      )
    : [];
        ...
        <CardContent>
          <ul className="space-y-2">
            {ProjectCategory &&
              ...
              ProjectCategory.slice(0, 5).map((category, index) => (
                <li key={index}>
                  <Link
                    href={`${paths.public.ongoingprojects}/${category.slug}`}
              ...
              <div key={post._id} className="flex gap-3">
                ...
                <div>
                  <h4 className="font-medium line-clamp-2 text-sm">
                    <Link
                      href={`${paths.public.ongoingprojects}/${post.slug}`}
...
export default ProjectDescriptionSidebar;
