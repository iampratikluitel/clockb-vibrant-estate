...
const Resources = () => {
  ...
  const handleDownload = async (doc: LegalDocument | InvestmentDoc) => {
    try {
      ...
      `/api/download?fileUrl=${encodeURIComponent(doc.fileUrl || "")}&downloadAs=${encodeURIComponent(doc.title)}`,
        {
          credentials: "include",
        }
      );
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {loadingGuides ? (
                ...
                guides.map((guide) => (
                  <div
                    key={guide.id}
                    className="bg-white rounded-xl shadow-md hover:shadow-xl transition-shadow p-6 border border-gray-100 flex flex-col"
                  >
